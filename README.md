...
│                   candidate-transformer --csv ... --json ...           │
└──────────────────────────────┬──────────────────────────────────────────┘
                               │
                               ▼
┌─────────────────────────────────────────────────────────────────────────┐
...
│                                                                         │
│  ┌──────────┐  ┌───────────┐  ┌───────────┐  ┌─────────┐  ┌────────┐  │
│  │ INGEST   │→ │ NORMALIZE │→ │   MATCH   │→ │  MERGE  │→ │ SCORE  │  │
│  │          │  │           │  │  (Entity   │  │(Conflict│  │(Conf.  │  │
│  │ CSV      │  │ Phone→E164│  │  Resolver) │  │ Resolve)│  │ Calc.) │  │
│  │ JSON     │  │ Email→lc  │  │           │  │         │  │        │  │
│  │ GitHub   │  │ Country→  │  │  email ∩   │  │ priority│  │ base × │  │
│  │ LinkedIn │  │   ISO3166 │  │  phone ∩   │  │  table  │  │ agree  │  │
│  │ PDF      │  │ Date→     │  │  name ≈    │  │  union  │  │ factor │  │
│  │ DOCX     │  │   YYYY-MM │  │           │  │  dedup  │  │        │  │
│  │ Notes    │  │ Skill→    │  └───────────┘  └─────────┘  └────────┘  │
│  │          │  │   canonical│                                          │
│  └──────────┘  └───────────┘                                          │
│                                                                         │
│  ┌────────────┐  ┌───────────┐  ┌──────────┐  ┌───────────────────┐    │
│  │ PROVENANCE │→ │  PROJECT  │→ │ VALIDATE │→ │      OUTPUT       │    │
│  │            │  │           │  │          │  │                   │    │
│  │ Track each │  │ Field     │  │ Schema   │  │ JSON to stdout    │    │
│  │ field's    │  │ selection │  │ check    │  │ or --output file  │    │
│  │ origin +   │  │ Rename    │  │          │  │                   │    │
│  │ transforms │  │ on_missing│  │          │  │                   │    │
│  └────────────┘  └───────────┘  └──────────┘  └───────────────────┘    │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```
...
# Run with coverage
pytest tests/ --cov=candidate_transformer --cov-report=term-missing

# Run a specific test module
pytest tests/test_normalizers.py -v

# Run a specific test
pytest tests/test_merge_engine.py::TestMergeEngine::test_merge_conflicting_names -v
```
